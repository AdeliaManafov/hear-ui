...
async def get_shap_explanation(request: ShapVisualizationRequest):
    ...
    try:
        ...
        # Try to compute SHAP values using the dedicated ShapExplainer (if available)
        try:
            from app.core.shap_explainer import ShapExplainer
            try:
                explainer = ShapExplainer(wrapper.model, feature_names=None, background_data=None, use_transformed=True)
                # Convert preprocessed DataFrame to numpy array for explainer
                if hasattr(preprocessed, 'values'):
                    sample_vals = preprocessed.values
                else:
                    sample_vals = np.array(preprocessed)

                shap_res = explainer.explain(sample_vals)
                # shap_res expected to be a dict with 'feature_importance', 'shap_values', 'base_value'
                if isinstance(shap_res, dict):
                    fi = shap_res.get('feature_importance', {})
                    sv = shap_res.get('shap_values', [])
                    bv = shap_res.get('base_value', 0.0)
                    # Normalize into our response structures
                    feature_importance = {k: float(v) for k, v in fi.items()} if fi else {f: 0.0 for f in EXPECTED_FEATURES}
                    shap_values = [float(x) for x in sv] if sv else [0.0] * len(feature_importance)
                    base_value = float(bv)
                else:
                    # Unexpected format: fall back to coefficient method below
                    raise ValueError("Unexpected shap_explainer.explain() return format")
            except Exception as e:
                logger.info("ShapExplainer failed or not available, falling back to coefficient-based contributions: %s", e)
                # Fallback to coefficient-based explanation below
                raise
        except Exception:
            # Coefficient-based explanation (existing fallback)
            try:
                model = wrapper.model
                ...
                logger.warning("Failed to compute coefficient-based feature importance: %s", e)
                feature_importance = {f: 0.0 for f in EXPECTED_FEATURES}
                shap_values = [0.0] * len(EXPECTED_FEATURES)
