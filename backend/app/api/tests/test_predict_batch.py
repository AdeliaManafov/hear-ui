...
def test_upload_csv_and_predict_basic():
    rows = [
        {"Alter": 45, "Seiten": "R", "Geschlecht": "m"},
        {"Alter": 30, "Seiten": "L", "Geschlecht": "w"},
    ]
...
def test_upload_empty_csv():
    ...
    resp = client.post(
        "/api/v1/patients/upload", files={"file": ("empty.csv", csv_bytes, "text/csv")}
    )
    assert resp.status_code == 200
    data = resp.json()
    assert data["count"] == 0
...
def test_upload_csv_all_empty_rows():
    ...
    resp = client.post(
        "/api/v1/patients/upload",
        files={"file": ("all_empty.csv", csv_content, "text/csv")},
    )
    assert resp.status_code == 200
    data = resp.json()
...
def test_upload_non_csv_file():
    ...
    resp = client.post(
        "/api/v1/patients/upload",
        files={"file": ("notcsv.txt", non_csv_content, "text/plain")},
    )
...
def test_upload_csv_with_bom():
    ...
    resp = client.post(
        "/api/v1/patients/upload", files={"file": ("bom.csv", csv_bytes, "text/csv")}
    )
    assert resp.status_code == 200
    data = resp.json()
...
def test_upload_csv_with_german_column_names():
    ...
    csv_bytes = _make_csv(rows)
    resp = client.post(
        "/api/v1/patients/upload", files={"file": ("german.csv", csv_bytes, "text/csv")}
    )
    assert resp.status_code == 200
    data = resp.json()
    assert data["count"] == 2
...
def test_upload_large_csv():
    ...
    rows = [
        {"Alter": 30 + i, "Geschlecht": "m" if i % 2 == 0 else "w", "Seiten": "R"}
        for i in range(100)
    ]
    csv_bytes = _make_csv(rows)
    resp = client.post(
        "/api/v1/patients/upload", files={"file": ("large.csv", csv_bytes, "text/csv")}
    )
    assert resp.status_code == 200
    data = resp.json()
...
def test_upload_csv_mixed_valid_invalid():
    ...
    csv_bytes = _make_csv(rows)
    resp = client.post(
        "/api/v1/patients/upload", files={"file": ("mixed.csv", csv_bytes, "text/csv")}
    )
    assert resp.status_code == 200
    data = resp.json()
    ...
    has_prediction = any(
        "prediction" in r and r["prediction"] is not None for r in data["results"]
    )
