...
CSV_PATH_HOST = (
    Path(__file__).parent.parent.parent / "patientsData" / "sample_patients.csv"
)
...
def main():
    if not CSV_PATH.exists():
        print("[FAIL] CSV file not found at either:")
        ...
        with open(CSV_PATH, encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                patient_create = PatientCreate(
                    input_features=features, display_name=display_name
                )

                try:
                    patient = crud.create_patient(
                        session=session, patient_in=patient_create
                    )
