...
\setlength{\parindent}{0pt}
  ...
  \begin{abstract}
    ...
    success rates. Our system combines a logistic regression model trained on 131 patient
    ...
    follows a RESTful architecture with comprehensive test coverage (52 test
    files, 394 test functions, 90\% coverage) and Docker-based deployment. Clinicians
  ...
  integrate AI insights with their clinical expertise, as shown in Figure~\ref{fig:prediction}.

  \begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/mainPredicitonView.png}
    \caption{Main prediction view: shows the predicted success probability and provides direct access to SHAP explanations to support clinical interpretation.}
  ...
  including local accuracy, missingness, and consistency, as demonstrated in our
  SHAP feature importance visualization (see Figure~\ref{fig:shap}).
  ...
  \begin{itemize}
    ...
    \item \textbf{Tests}: 52 test files with 394 test functions covering unit,
  ...
  \end{itemize}
  as illustrated in Figure~\ref{fig:prediction}, Figure~\ref{fig:shap}, and Figure~\ref{fig:feedback}.
  ...
  The model outputs a probability $P(success | X)$ where $X$ represents the 131-dimensional
  ...
  actionable insights, as shown in Figure~\ref{fig:shap}.

  \begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/shap_feature_importance.png}
    \caption{SHAP feature weighting chart of the most influential features on the predicted probability. 
    The bar length indicates the strength of the respective contribution, while the color encodes the direction 
    of the influence. The chart is presented as a positive-negative bar graph.}
    \label{fig:shap}
  \end{figure*}
  ...
  \begin{itemize}
    ...
    \item Continuous quality improvement of both predictions and explanations,
      as shown in Figure~\ref{fig:feedback}
  \end{itemize}

  \begin{figure*}[t]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering
    \textbf{Placeholder for Feedback Interface Screenshot}\\
    \vspace{2cm} % Space for screenshot
    }}
  ...
  \begin{itemize}
    \item \textbf{Unit tests}: 394 test functions across 52 test files covering model
    ...
    \item \textbf{Code quality}: Automated linting and formatting with Ruff (backend)
      and ESLint (frontend), achieving 90\% test coverage
  ...
  \begin{itemize}
    ...
    \item \textbf{SHAP consistency}: Feature importance rankings consistently align
      ...
      \end{itemize}
      as visualized in Figure~\ref{fig:shap}.
  ...
  \begin{itemize}
    ...
    \item \textbf{API reliability}: All 202 tests passing with 90\% code
  ...
  and patient-specific), a comprehensive test infrastructure (394 test functions in
  52 test files with 90 percent test coverage), the complete setup of the CI/CD pipeline
  ...
  delete). Sie trug auch zur Systemdokumentation bei.
