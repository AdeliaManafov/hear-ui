...
\begin{document}
  ...
  (5) imaging findings, (6) audiological measurements (LL, 4000~Hz thresholds),
  and (7) implant specifications (manufacturer, electrode type). The API endpoint
  (\texttt{POST /api/v1/predict/}) accepts both German column names (e.g.,
  ``Alter~[J]'') and English aliases (e.g., ``age'') for flexibility.
  ...
  \begin{itemize}
    \item \textbf{Prediction range}: Model outputs span 0.22--1.0. While 1.0 is
      an extreme value, it represents legitimate high-confidence predictions for
      patients with strongly favorable profiles. The absence of predictions below 0.22
      reflects the preprocessor's default values (e.g., age=50, abstand=365 days)
      which establish a baseline probability floor. This range indicates the model
      provides meaningful differentiation across patient profiles without overfitting
      to extremes
  ...
  the SHAP and gathered frontend feedback. He was responsible for creating
  the demo video.
