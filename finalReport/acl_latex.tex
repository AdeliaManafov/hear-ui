...
\begin{document}
  ...
  Required clinician inputs include demographics (age, gender), pre-operative speech
  scores, imaging findings, hearing history, and implant details. These are accepted
  by the API's prediction endpoint (\texttt{POST /api/v1/predict/}) and
  normalized by the backend preprocessor.
  ...
  \begin{itemize}
    \item \textbf{Prediction range}: Model outputs mostly span 0.22--0.99, with occasional
      exact 1.0 values for high-confidence cases. While 1.0 is technically an
      extreme value, the model avoids clustering predictions at 0.0--0.21, indicating
      reasonable calibration and conservative thresholding
