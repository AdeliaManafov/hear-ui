...
\begin{document}
  ...
  \begin{abstract}
    ...
    success rates. Our system combines a logistic regression model trained on
    131 patient features with SHAP-based explanations to provide transparent,
    interpretable predictions. The web application, built with FastAPI (Python)
    and Vue.js (TypeScript), follows a RESTful architecture with comprehensive
    test coverage and Docker-based deployment. Clinicians can input patient data and receive both
    a success probability and feature importance rankings that explain the
    prediction. Our approach addresses the critical need for transparency in
    medical AI systems by providing human-understandable explanations for each
    prediction, while maintaining production-grade code quality through
  ...
  In this paper, we present HEAR-UI, an AI-powered clinical decision support
  system that addresses these challenges by:
  ...
  Our approach follows the principle that medical AI systems must be inherently
  interpretable, not merely accurate \citep{rudin2019stop}. By integrating SHAP
  (SHapley Additive exPlanations) \citep{lundberg2017shap} as a core system
  component rather than a post-hoc addition, we ensure clinicians can understand
  \emph{why} the model makes specific predictions, enabling informed integration
  of AI insights with clinical expertise.
  ...
  \begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/shap_feature_importance.png}
    \caption{SHAP feature importance view showing the top contributors to the predicted probability; color encodes direction of effect and bar length encodes contribution magnitude.}
    \label{fig:shap_viz}
  \end{figure*}
  ...
  The deployment of AI in clinical settings has highlighted the critical need for
  interpretable systems. \citet{shortliffe2018clinical} emphasizes that clinical
  decision support systems must provide explanations that clinicians can evaluate
  and potentially override. \citet{topol2019medicine} discusses the convergence of
  AI and medicine, noting that transparency is essential for clinical adoption and
  regulatory compliance.

  \textbf{Explanation Methods:} SHAP values \citep{lundberg2017shap} have emerged
  as a leading approach for model-agnostic interpretability, providing theoretically
  grounded feature importance measures based on Shapley values from cooperative
  game theory. Unlike LIME \citep{ribeiro2016lime}, which provides locally faithful
  but potentially unstable approximations, SHAP values satisfy critical properties:
  \emph{local accuracy} (explanation sums to model output), \emph{missingness}
  (absent features have zero contribution), and \emph{consistency} (monotonic
  relationship between feature effect and contribution).

  \textbf{Clinical Adoption:} \citet{wiens2019nostudy} provide guidelines for
  responsible ML in healthcare, emphasizing interpretable models, prospective
  validation, and clinician feedback mechanisms. Recent work has shown that
  explanation quality significantly impacts clinical trust. Our system incorporates
  these insights through interactive visualizations and feedback collection
  ...
  Raw patient data undergoes preprocessing to create a 131-dimensional feature
  ...
  TypeScript, Vite build system), FastAPI backend (Python 3.12), and PostgreSQL 12
  ...
  the modular structure facilitates maintenance and future extensions.
  
  \begin{figure}[!htbp]
    \centering
    \fbox{\parbox{0.9\columnwidth}{\centering
    \textbf{Frontend} (Vue 3 + TypeScript)\\
    \small{Interactive SHAP visualizations, patient forms}\\
    $\downarrow$ REST API $\downarrow$\\
    \textbf{Backend} (FastAPI + Python)\\
    \small{API endpoints, data validation}\\
    $\downarrow$\\
    \textbf{ML Pipeline} (LogReg + SHAP)\\
    \small{Prediction ($<$100ms) + Explanation ($<$500ms)}\\
    $\downarrow$\\
    \textbf{Database} (PostgreSQL)\\
    \small{Patients, predictions, feedback} }}
    \caption{HEAR-UI system architecture showing the three-tier design with ML
    pipeline integration and real-time explanation generation.}
  ...
  \begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/mainPredicitonView.png}
    \caption{Main prediction view: shows the predicted success probability and
    provides direct access to SHAP explanations to support clinical interpretation.}
    \label{fig:ui}
  \end{figure*}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Baseline adequacy}: Given the 131-dimensional feature space and limited
  ...
  \begin{figure*}[t]
    \centering
    % Show `ui_feedback.png` if present, otherwise keep placeholder
    \IfFileExists{figures/ui_feedback.png}{%
      \includegraphics[width=\textwidth]{figures/ui_feedback.png}
    }{%
      \fbox{\parbox{0.9\columnwidth}{\centering\vspace{0.6cm}
      \textbf{[Screenshot: Feedback Interface]}\\[0.5em]
      \small{Clinicians record agreement with predictions and explanation
      quality; responses are stored for later explanation evaluation.}
      \vspace{0.6cm}}}
    }
    \caption{Feedback interface for clinicians to record agreement with predictions and rate explanation usefulness; used to collect data for explanation evaluation.}
    \label{fig:feedback}
  \end{figure*}

  The model outputs a probability $P(success \mid X)$ where $X$ represents the 131-dimensional
  feature vector. This probability is computed via the logistic function:

  \begin{equation}
    P(success \mid X) = \frac{1}{1 + e^{-(\beta_0 + \beta^{\top} X)}}
  \end{equation}
  ...
  For each prediction, we compute SHAP values to decompose the model output into
  individual feature contributions. For a prediction $f(x)$, SHAP values $\phi_{i}$
  satisfy the additive decomposition:

  \begin{equation}
    f(x) = \phi_{0}+ \sum_{i=1}^{M}\phi_{i}
  \end{equation}

  where $\phi_{0}$ is the expected model output (base value) computed across the
  training distribution, and $\phi_{i}$ represents the contribution of feature $i$
  to shifting the prediction from the base value to the final output.

  \textbf{Implementation:} Our system uses SHAP's LinearExplainer for the logistic
  regression model, providing exact Shapley values in $O(M)$ time complexity where
  $M=131$ features. This enables real-time explanation generation ($<$500ms per
  prediction). We also support KernelExplainer for model-agnostic fallback,
  though with increased computational cost.

  \textbf{Visualization Strategy:} The web interface presents SHAP explanations
  through multiple complementary views (Figure~\ref{fig:shap_viz}):
  \begin{itemize}
    \item \textbf{Feature importance ranking}: Top-5 features sorted by absolute
      SHAP value magnitude
    \item \textbf{Contribution direction}: Color-coded indicators showing whether
      features increase (green) or decrease (red) success probability
    \item \textbf{Numeric values}: Actual SHAP contributions and patient feature values
      for full transparency
  \end{itemize}

  This multi-level presentation allows clinicians to quickly grasp key drivers
  while retaining access to detailed numeric information for deeper analysis.

  \subsection{API Design}

  The backend exposes RESTful endpoints for all functionality:

  % Reduce vertical spacing between these items for compact display
  ...
  internally uses 131 engineered features, clinicians need only provide the available
  ...
  We evaluate both \emph{prediction accuracy} (model performance) and \emph{explanation
  quality} (SHAP interpretability), recognizing that clinical utility requires
  ...
  \begin{itemize}
    \item \textbf{Tests}: Comprehensive automated test suite covering unit, integration,
      and end-to-end scenarios; CI runs tests and linters on every commit

    \item \textbf{Code quality}: Automated linting and formatting with Ruff (backend)
      and ESLint (frontend); test coverage reported in the CI

    \item \textbf{Containerization \& CI/CD}: Docker Compose for reproducible local
      deployments and GitHub Actions for pipeline automation
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item \textbf{Our approach}: Logistic regression with 131 engineered features provides
  ...
  \begin{itemize}
    ...
    \item \textbf{Preprocessing validation}: 131 features correctly encoded through
    ...
    \item \textbf{Robustness testing}: Model handles missing values through sensible
      defaults and flexible input normalization, validated through 394 unit and
      integration tests
  \end{itemize}
  ...
  \label{sec:explanation_eval}

  Evaluating explanation quality is central to our XAI contribution. We assess
  SHAP explanations across multiple dimensions:

  \textbf{1. Faithfulness and Consistency}
  \begin{itemize}
    \item \textbf{Additive decomposition}: Verify that $\sum_i \phi_i + \phi_0 = f(x)$
      holds numerically (tolerance $<10^{-6}$)
    \item \textbf{Monotonicity}: Features identified as positive contributors should
      increase prediction when their values increase (tested via perturbation)
    \item \textbf{Stability}: Consistent feature rankings across similar patients
      (measured via rank correlation)
  \end{itemize}

  \textbf{2. Clinical Plausibility}
  \begin{itemize}
    \item \textbf{Literature alignment}: Top features match published prognostic factors
      \citep{blamey2013factors}: pre-operative scores, duration of deafness, age
    \item \textbf{Direction consistency}: Sign of SHAP values aligns with known
      clinical relationships (e.g., longer deafness $\rightarrow$ worse outcomes)
    \item \textbf{Feature interactions}: Examination of feature dependence plots for
      non-linear effects
  \end{itemize}

  \textbf{3. Computational Efficiency}
  \begin{itemize}
    \item \textbf{Explanation latency}: SHAP computation $<$500ms per prediction
      (measured across 1000+ samples)
    \item \textbf{Scalability}: Batch explanation generation for multiple patients
    \item \textbf{Resource usage}: Memory and CPU overhead acceptable for production
      deployment
  \end{itemize}

  \textbf{4. User Interface Effectiveness}
  \begin{itemize}
    \item \textbf{Visual clarity}: SHAP bar charts render correctly with proper
      feature labels (German/English)
    \item \textbf{Information density}: Top-5 features provide actionable insights
      without overwhelming clinicians
    \item \textbf{Feedback collection}: Implemented; see Section~\ref{sec:method} for details
  ...
  \begin{itemize}
    ...
    \item \textbf{API reliability}: All 394 tests passing with 90\% code
  ...
  Table~\ref{tab:predictions}  in Section 6 shows example predictions for sample patients,
  ...
  The SHAP explanations reveal clinically meaningful and literature-consistent
  patterns (Figure~\ref{fig:shap_viz}):

  \textbf{Top Contributing Features:}
  \begin{itemize}
    \item \textbf{Pre-operative speech scores}: Higher pre-op scores consistently show
      positive SHAP values (mean $\phi = +0.15$), aligning with \citet{blamey2013factors}
      findings that baseline performance predicts outcomes

    \item \textbf{Duration of profound hearing loss}: Longer duration yields negative
      contributions (mean $\phi = -0.12$ for $>$10 years), reflecting auditory
      deprivation effects

    \item \textbf{Age at implantation}: Non-linear relationship captured through one-hot
      encoding; younger patients ($<$50 years) show positive contributions

    \item \textbf{Implant type}: MED-EL vs. Cochlear electrodes show differential effects
      ($\Delta\phi \approx 0.08$), potentially reflecting device-specific performance
  \end{itemize}

  \textbf{Explanation Consistency:} Across 100+ sample patients, SHAP rankings
  exhibit high stability (mean Kendall's $\tau = 0.82$ for top-5 features between
  similar patients). This consistency is crucial for clinical trust---explanations
  should not radically change for minor variations in patient data.

  \textbf{Visualization Effectiveness:} Visualizations provide at-a-glance
  interpretation of feature contributions; the UI design and examples are
  described in Section~\ref{sec:method} and Figure~\ref{fig:ui}.
  ...
  \begin{enumerate}
    ...
    \item \textbf{Model simplicity}: While interpretable, logistic regression
      may miss complex non-linear relationships. See Section~\ref{sec:method} for the
    ...
    \item \textbf{Database schema rigidity}: We used PostgreSQL with a fixed relational
      ...
      for our use case, where the 131-dimensional feature vector is stored as JSON
  ...
  and refining explanations of system architecture and machine learning concepts. Examples include
  summarizing technical descriptions or rephrasing explanations for clarity. No generative AI was
  used to produce code, experimental results, or system design decisions.
  ...
  and patient-specific), a comprehensive test infrastructure, the complete setup of the CI/CD pipeline
  ...
  integration with the backend API and was responsible for the creation of the demo video.
  ...
  tasks on the SHAP and gathered frontend feedback.He was also responsible for creating the 
  demo video and contributed to the system documentation.
  ...
  The complete list of 131 model features is organized into:
  ...
  \begin{itemize}
    \item \textbf{Backend}: Python 3.12, FastAPI 0.128.0, scikit-learn 1.6.1, SHAP 0.50.0, SQLModel 0.0.31, Alembic 1.18.1
    
    \item \textbf{Database}: PostgreSQL 12
