...
HEAR-UI follows a three-tier architecture consisting of a Vue.js 3 frontend (with TypeScript, Vite build system), FastAPI backend (Python 3.12+), and PostgreSQL 12 database, all containerized using Docker Compose for reproducibility and deployment flexibility. The system is deployed using a classical web architecture where the frontend and backend communicate via a RESTful API. Figure~\ref{fig:architecture} illustrates the system components.
...
\begin{itemize}
    ...
    \item \textbf{Tests}: 43 test modules with unit, integration, and API tests
\end{itemize}
...
\begin{itemize}
    \item \textbf{Unit tests}: 183+ tests across 43 test modules covering model wrapper, preprocessor, SHAP explainer, and all core functionality
...
\begin{itemize}
    \item \textbf{Prediction range}: Model outputs span 0.22--1.0 probability range, avoiding extreme values and indicating reasonable calibration
    \item \textbf{SHAP consistency}: Feature importance rankings consistently align with clinical domain knowledge
    \begin{itemize}
        \item Pre-operative speech recognition scores show highest impact
    ...
    \end{itemize}
    \item \textbf{Preprocessing validation}: 68 features correctly encoded through one-hot encoding, binary flags, and numeric normalization
    \item \textbf{Robustness testing}: Model handles missing values through sensible defaults and flexible input normalization
\end{itemize}
...
A.~Manafov led the backend development (FastAPI architecture, RESTful API design), ML pipeline integration (model wrapper, preprocessor), SHAP implementation, and database schema design. Contributions include the core prediction and explanation endpoints, comprehensive API testing (40+ endpoint tests), and Docker orchestration setup.

A.~Mozharov was responsible for frontend development (Vue.js 3 with TypeScript), UI/UX design, patient data input forms, prediction visualization components, and SHAP feature importance charts. Implemented Playwright E2E tests and frontend integration with the backend API.

N.~Kuhl contributed to data preprocessing pipeline design, feature engineering strategy, testing infrastructure (pytest configuration, CI/CD setup with GitHub Actions), code quality tooling (Ruff, ESLint), and comprehensive project documentation. All authors collaborated on system architecture decisions, evaluation methodology, and manuscript preparation. Equal contribution to debugging, code reviews, and deployment validation.
